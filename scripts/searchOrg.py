...
def get_org_repos(organization_name, page) -> str:
    response = get(url='https://api.github.com/users/' + organization_name + '/repos?page={}'.format(page))
    json = response.json()
    if not json:
        return ""
    result = ""
    for item in json:
        if not item['fork']:
            ...
            results = truffleHog.find_strings(
                item["html_url"],
    ...
    result += get_org_repos(organization_name, page + 1)
    return result
