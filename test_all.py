...
class TestStringMethods(unittest.TestCase):
    ...
    def test_return_correct_commit_hash(self):
        ...
        filtered_results = list(filter(
            lambda r: r['commitHash'] == commit_w_secret and r['branch'] == 'origin/master', results
        ))
    ...
    def test_path_included(self):
        ...
        dest_paths = {blob.b_path for blob in blobs.values() if blob.b_path is not None}
        ...
        for name, blob in blobs.items():
            ...
            self.assertFalse(truffleHog.path_included(
                blob, include_patterns=overlap_patterns,
                exclude_patterns=all_paths_patterns),
                '{} should be excluded with overlapping patterns: \n\tinclude: {}\n\texclude: {}'.format(
            ...
            if name.startswith('deleted-file-'):
                self.assertTrue(
                    truffleHog.path_included(blob, include_patterns=deleted_paths_patterns),
                    '{}: inclusion should match deleted paths: {}'.format(blob, deleted_paths_patterns))
                self.assertFalse(
                    truffleHog.path_included(blob, exclude_patterns=deleted_paths_patterns),
